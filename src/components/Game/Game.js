...
class Timer extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <div
        className={`timer ${remaining < 5 && remaining > 0 ? "animated bounceIn red" : ""}`}
  ...
  componentDidUpdate() {
    setTimeout(() => {
      ...
      if (this.change) {
        ...
        node.className += change > 0 ? " positive fadeOutUp" : " negative fadeOutDown";
      }
    }, 0);
  }
        ...
        <div className="container">
          ...
          <div className="status">
            ...
            <div className="status-item score">
              {this.format(score)}
              {multiplier > 1 && <div className="multiplier">x{multiplier}</div>}
              {change !== 0 && (
                <div ref={(ref) => (this.change = ref)} className="change animated">
                  {change > 0 ? "+" : ""}
                  {change}
                </div>
              )}
    ...
    return (
      <div className="multiple-choice">
        {values.map((res, index) => (
          <div
            className={`choice animated ${selected === res ? (correct ? "tada positive" : "negative wobble") : ""}`}
            onClick={() => onClick(res)}
            key={index}
          >
            {res}
